...
uint32_t	pixel_color(mlx_texture_t *texture, t_data *data, int t_pix)
{
    ...
    if (data->ray->is_wall)
        x_pixel_coordinate = (int)fmodf((data->ray->horizontal_x * (texture->width / TILE_SIZE)), texture->width);
    else
        x_pixel_coordinate = (int)fmodf((data->ray->vertical_y * (texture->width / TILE_SIZE)), texture->width);

	pixel_array = (uint32_t *)texture->pixels;
	factor = (double)texture->height / data->ray->wall_h;
	y_pixel_coordinate = (t_pix - (WINDOW_HEIGHT / 2) + (data->ray->wall_h / 2)) * factor;
...
void	render_wall(t_data *data)
{
	int				t_pix;
	uint32_t		color;
	mlx_texture_t	*texture = data->ray->current_texture;
