...
//------------------------former main from leonard/textures
// branch------------------------//
...
void	move_player(t_mlx *mlx, double move_x, double move_y) // move the player
{
	...
	new_x = roundf(mlx->ply->plyr_x
		+ move_x);                                                                                                                                                
		// get the new x position
	new_y = roundf(mlx->ply->plyr_y
		+ move_y);                                                                                                                                                
...
int	wall_hit(float x, float y, t_mlx *mlx) // check the wall hit
{
	...
	if ((y_m >= mlx->dt->h_map || x_m >= mlx->dt->width))
		return (0);
...
float	get_h_inter(t_mlx *data, float angl) // get the horizontal intersection
{
	...
	data->ray->horizontal_x = h_x;                                                   
		// h_x: The x-coordinate of the point where the
																						// horizontal ray intersects a wall.
	data->ray->horizontal_y = h_y;                                                   
		// h_y: The y-coordinate of the point where the
																						// horizontal ray intersects a wall.
	return (sqrt(pow(h_x - data->ply->plyr_x, 2) + pow(h_y - data->ply->plyr_y,
				2))); // get the distance
}
...
float	get_v_inter(t_mlx *data, float angl) // get the vertical intersection
{
	...
	return (sqrt(pow(v_x - data->ply->plyr_x, 2) + pow(v_y - data->ply->plyr_y,
				2))); // get the distance
}
...
void	cast_rays(t_data *data) // cast the rays
{
	...
	data->ray->screen_x = 0;
	data->ray->angle_rd = data->ply->angle_rd - (data->ply->fov_rd / 2);
		// the start angle
	while (data->ray->screen_x < S_W)                                   
		// loop for the rays
	{
		data->ray->is_wall = 0; // flag for the wall
		h_inter = get_h_inter(data,
								nor_angle(data->ray->angle_rd));      
...
void	start_the_game(t_map *dt, char *map_argv) // start the game
{
	...
	mlx.ply = calloc(1, sizeof(t_player));     
		// init the player structure i'm using calloc
...
t_map	*init_argumet(void) // init the data structure
{
