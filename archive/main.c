...
float get_h_inter(t_mlx *data, float angl) // get the horizontal intersection
{
  ...
  data->ray->horizontal_x = h_x; // h_x: The x-coordinate of the point where the
                                 // horizontal ray intersects a wall.
  data->ray->horizontal_y = h_y; // h_y: The y-coordinate of the point where the
                                 // horizontal ray intersects a wall.
  return (sqrt(pow(h_x - data->ply->plyr_x, 2) +
               pow(h_y - data->ply->plyr_y, 2))); // get the distance
}
...
float get_v_inter(t_mlx *data, float angl) // get the vertical intersection
{
  ...
  return (sqrt(pow(v_x - data->ply->plyr_x, 2) +
               pow(v_y - data->ply->plyr_y, 2))); // get the distance
}
...
void cast_rays(t_data *data) // cast the rays
{
  ...
  data->ray->screen_x = 0;
  data->ray->angle_rd =
      data->ply->angle_rd - (data->ply->fov_rd / 2); // the start angle
